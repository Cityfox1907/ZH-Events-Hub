...
// Only places (no events) — events live in /entdecken/kalender
const PLACES_ONLY = LISTINGS_ALL.filter((l) => l.kind === "place");

export default function OrteSubPage() {
  return (
...
function OrteInner() {
  ...
  useEffect(() => {
    ...
    const next = qs ? `/entdecken/orte?${qs}` : "/entdecken/orte";
    router.replace(next, { scroll: false });
  }, [activeCat, activeStyle, district, router]);

  const filtered: Listing[] = useMemo(() => {
    let list = PLACES_ONLY.slice();
    ...
    if (sort === "rating") {
      ...
      list.sort((a, b) => {
        const aScore = (a.trending ? 50 : 0) + (a.rating ?? 0) * 10;
        const bScore = (b.trending ? 50 : 0) + (b.rating ?? 0) * 10;
        return bScore - aScore;
      });
    }

    return list;
  }, [query, activeCat, activeStyle, district, minRating, sort]);

  const trending24h = useMemo(
    () =>
      PLACES_ONLY.filter((l) => l.trending || (l.views_24h ?? 0) > 100).slice(0, 6),
    [],
  );

  const districtSpotlight = useMemo(
    () =>
      PLACES_ONLY.filter((l) => l.district === DISTRICT_SPOTLIGHT.district).slice(
        0,
        6,
      ),
    [],
  ...
  return (
    <>
      <section className="container-editorial pt-10 pb-6">
        <p className="eyebrow">Entdecken · Orte</p>
        <h1 className="font-display text-4xl md:text-6xl mt-2 leading-[0.95] tracking-tight">
          Restaurants, Bars, Badis,<br />
          <span className="italic text-burgundy">Aktivitäten.</span>
        </h1>
        ...
        <div className="mt-6 max-w-xl relative">
          ...
          <input
            ...
            placeholder="Restaurant, Bar, Badi, Museum suchen…"
      ...
      <section className="container-editorial pt-5 pb-3">
        {(activeCat || activeStyle || district || minRating) && (
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <p className="text-[12.5px] text-ink-muted">
            <strong className="text-ink">{filtered.length}</strong> Orte
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5">
          {CURATED_THEMES.map((t) => {
            ...
            return (
              <Link
                key={t.key}
                href={`/entdecken/orte?${sp.toString()}`}
...
function ListingCard({ listing }: { listing: Listing }) {
  return (
    <Link
      href={listing.href}
      className="group relative block bg-card rounded-2xl overflow-hidden border border-line card-shadow card-shadow-hover transition-shadow border-l-2 border-l-brass"
    >
